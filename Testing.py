...
def evaluate_detection(image_paths, ground_truth_boxes, test_size=0.2):
    """
    Evaluate the detection and classification system
    """
    all_ious = []
    component_features = []
    component_labels = []
        ...
        try:
            ...
            for component in components:
                ...
                if features is not None:
                    ...
                    _, bbox = component
                    max_iou = max(calculate_iou(bbox, gt_box) for gt_box in ground_truth_boxes[idx])
                    all_ious.append(max_iou)
                    component_labels.append(1 if max_iou > 0.5 else 0)
...
def main():
    ...
    # Example ground truth boxes for each image
    ground_truth_boxes = [
        [(100, 100, 50, 50), (200, 200, 50, 50)]  # Example for each image
        for _ in range(20)
    ]
    
    # Evaluate the system
    metrics = evaluate_detection(image_paths, ground_truth_boxes)
